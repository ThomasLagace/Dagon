...
class UserBase {
    ...
    public function __construct($login) {
        global $db;
        $this->login = $login;
        $q = $db->prepare("SELECT name, avatar, id FROM utilizers WHERE login = (:login)");
        $q->bindParam(':login', $login);
        $q->execute();
        if ($q->rowCount() > 0) {
            $r = $q->fetch(PDO::FETCH_ASSOC);
            if ( is_null($r['name']) ) {
                $this->username = $r['login'];
            }
    ...
    public function register($password, $code) {
        ...
        # Jack 'em in (thx jack)
        createSession($this->login, $r['lvl']);
        return;
    }
    ...
    public function login($password) {
        global $db;
        # Get user's data
        $q = $db->prepare("SELECT id, password, login, lvl FROM utilizers WHERE login = (:login)");
        $q->bindParam(':login', $this->login);
        $q->execute();
        $r = $q->fetch();
